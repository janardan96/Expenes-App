import { Text, TextStyle, Platform, StyleSheet } from 'react-native';
import React, { FC } from 'react';
import { RFValue } from 'react-native-responsive-fontsize';

type Variant = 'h1' | 'h2' | 'h3' | 'h4' | 'h5' | 'h6' | 'h7' | 'h8';
type PlatformType = 'android' | 'ios';

interface CustomTextProps {
  variant?: Variant;
  fontSize?: number;
  style?: TextStyle | TextStyle[];
  children?: React.ReactNode;
  numberOfLines?: number;
  onLayout?: (event: any) => void;
  className?: string; // Optional className prop for Tailwind CSS
}

const fontSizeMap: Record<Variant, Record<PlatformType, number>> = {
  h1: { android: 24, ios: 22 },
  h2: { android: 22, ios: 20 },
  h3: { android: 20, ios: 18 },
  h4: { android: 18, ios: 16 },
  h5: { android: 16, ios: 14 },
  h6: { android: 14, ios: 12 },
  h7: { android: 12, ios: 10 },
  h8: { android: 10, ios: 9 },
};

const CustomText: FC<CustomTextProps> = ({
  variant,
  fontSize,
  style,
  children,
  numberOfLines = undefined,
  onLayout,
  className, // Accept className prop for Tailwind CSS
  ...props
}) => {
  let computedFontSize: number =
    Platform.OS === 'android'
      ? RFValue(fontSize || 12)
      : RFValue(fontSize || 10);

  if (variant && fontSizeMap[variant]) {
    const defaultSize = fontSizeMap[variant][Platform.OS as PlatformType];
    computedFontSize = RFValue(fontSize || defaultSize);
  }

  return (
    <Text
      onLayout={onLayout}
      numberOfLines={numberOfLines}
      style={[styles.text, { fontSize: computedFontSize }, style]}
      className={className} // Apply Tailwind CSS classes if provided
      {...props}
    >
      {children}
    </Text>
  );
};

export default CustomText;

const styles = StyleSheet.create({
  text: {
    textAlign: 'left',
  },
});
